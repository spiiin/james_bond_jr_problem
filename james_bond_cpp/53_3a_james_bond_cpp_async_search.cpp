...
std::vector<MatrixHashT> bidirectionalSearch(const Matrix& source, const Matrix& target) {
    ...
    while (!openForward.empty() && !openBackward.empty()) {
        auto expand = [&](OpenList& open, ClosedList& closed, ClosedList& oppositeClosed) -> std::optional<MatrixHashT> {
            ...
            std::array<std::future<ClosedList::iterator>, 16> searchFutures;
            size_t searchFuturesIndex = 0u;
            for (const auto& [f, line] : fun) {
                MatrixHashT valHash = f(head, line);
                searchFutures[searchFuturesIndex++] = std::async(std::launch::async, [&]() {
                    return oppositeClosed.find(valHash);
                });
            ...
            for (auto& searchFuture : searchFutures) {
                auto futureIt = searchFuture.get();
                if (futureIt != oppositeClosed.end()) {
                    closed[futureIt->first] = head;
                    return futureIt->first;
                }
            }
            return std::nullopt;
        };
