...
using BloomFilter = bloom_filter;
...
std::vector<MatrixHashT> bidirectionalSearch(const Matrix& source, const Matrix& target) {
    ...
    parameters.projected_element_count = BUCKETS_COUNT;
    parameters.false_positive_probability = 0.001;
    parameters.random_seed = 0xA5A5A5A5;
    parameters.compute_optimal_parameters();
    static BloomFilter bloomForward(parameters);
    static BloomFilter bloomBackward(parameters);

    while (!openForward.empty() && !openBackward.empty()) {
        auto expand = [&](OpenList& open, ClosedList& closed, ClosedList& oppositeClosed, BloomFilter& bloomOpposite) -> std::optional<MatrixHashT> {
            auto head = open.pop();

            for (const auto& [f, line] : fun) {
                MatrixHashT valHash = f(head, line);
                if (bloomOpposite.contains(valHash) && oppositeClosed.find(valHash) != oppositeClosed.end()) {
                    closed[valHash] = head;
                    return valHash;
                }
                if (closed.emplace(valHash, head).second) {
                    open.push(valHash);
                    bloomOpposite.insert(valHash);
                }
            }
            return std::nullopt;
        };

        if (auto meetingPoint = expand(openForward, closedForward, closedBackward, bloomBackward)) {
            return extractPath(closedForward, closedBackward, *meetingPoint);
        }

        if (auto meetingPoint = expand(openBackward, closedBackward, closedForward, bloomForward)) {
            return extractPath(closedForward, closedBackward, *meetingPoint);
        }
    }
