...
using ClosedList = boost::unordered::concurrent_flat_map<MatrixHashT, MatrixHashT, MurMurHasher>;
using OpenHash = boost::unordered::concurrent_flat_set<MatrixHashT, MurMurHasher>;
using OpenList = boost::lockfree::queue<Node>;
...
std::vector<MatrixHashT> extractPath(const ClosedList& closedForward, const ClosedList& closedBackward, const MatrixHashT& meetingPoint) {
    ...
    do {
        found = false;
        closedForward.visit(current, [&](const auto& value) {
    ...
    do {
        found = false;
        closedBackward.visit(current, [&](const auto& value) {
            current = value.second;
            pathForward.push_back(current);
            found = true;
        });
...
static std::vector<MatrixHashT> bidirectionalSearch(const Matrix& source, const Matrix& target) {
    static auto fun = generateFunctions();
    static auto source_hash = MatrixHash{}(source);
    static auto target_hash = MatrixHash{}(target);

    OpenList openForward(4096u), openBackward(4096u);
    openForward.push({ source_hash, rate(source_hash, target_hash), 0 });
    openBackward.push({ target_hash, rate(target_hash, source_hash), 0 });
    
    OpenHash openHashForward;
    openHashForward.emplace(source_hash);
    OpenHash openHashBackward;
    openHashBackward.emplace(target_hash);
    ...
    std::vector<std::vector<MatrixHashT>> results(THREAD_COUNT);
    std::vector<std::thread> threads;

    auto thread_func = [&](int threadIndex) -> void {
        while (!openForward.empty() && !openBackward.empty() && !found.load()) {
            auto expand = [&](OpenList& open, OpenHash& openHash, ClosedList& closed, ClosedList& oppositeClosed) -> std::optional<MatrixHashT> {
                Node head;
                if (!open.pop(head)) return std::nullopt;

                for (const auto& [f, line] : fun) {
                    MatrixHashT valHash = f(head.matrix, line);
                    uint8_t newPathLength = head.path_length + 1;

                    bool existsInOpposite = false;
                    oppositeClosed.cvisit(valHash, [&](const auto&) {
                        existsInOpposite = true;
                        });

                    if (existsInOpposite) {
                        closed.emplace(valHash, head.matrix);
                        meetingPoint = valHash;
                        found.store(true);
                        return valHash;
                    }

                    if (openHash.emplace(valHash)) {
            ...
            if (auto meeting = expand(openForward, openHashForward, closedForward, closedBackward)) {
                results[threadIndex] = extractPath(closedForward, closedBackward, *meeting);
                found.store(true);
                return;
            }

            if (auto meeting = expand(openBackward, openHashBackward, closedBackward, closedForward)) {
                results[threadIndex] = extractPath(closedForward, closedBackward, *meeting);
                found.store(true);
                return;
            }
