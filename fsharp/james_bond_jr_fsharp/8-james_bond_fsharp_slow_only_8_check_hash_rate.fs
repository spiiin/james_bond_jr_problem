...
let rec nextSearchStep (transformPaths: Path array -> Path array) (filterPath: Path -> bool) (paths: Path array) (explored: ConcurrentDictionary<State, int>) (endState: State) : seq<Path array> = 
    seq {
        if Array.isEmpty paths then
            ...
            more |> PSeq.iter (fun path -> tryAddOrUpdate explored path.State path.DiffCount |> ignore)
...
let transformPaths (paths: Path array) : Path array =
    paths |> Array.sortBy (fun path -> path.DiffCount) |> Array.truncate 200000
