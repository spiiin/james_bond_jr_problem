...
let rate (State s1) (State s2) : int =
    ...
    for i = 0 to 15 do
        if s1.[i] <> s2.[i] then
            let row, col = i / 4, i % 4
            let target = s2.[i]
            let mutable rowShifts = System.Int32.MaxValue
            for j = 0 to 3 do
                if s1.[row * 4 + j] = target then
                    rowShifts <- min rowShifts (getWrapDistance j col 4)
            let mutable colShifts = System.Int32.MaxValue
            for j = 0 to 3 do
                if s1.[j * 4 + col] = target then
            ...
            let shift =
                ...
                | rs, System.Int32.MaxValue -> rs
                | System.Int32.MaxValue, cs -> cs
                | rs, cs -> min rs cs
            total <- total + shift
    total
