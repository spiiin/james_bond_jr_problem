...
let rec next_search_step (paths: Path list) (explored: HashSet<State>) (endState: State) (maxPathLength: int): seq<Path list> =
    seq {
        if List.isEmpty paths then
            ...
            let sortedPaths = paths |> List.sortWith comparePaths |> List.truncate 200000 //TODO: 1. truncate-func -> argument. 2. partial-sort?
            ...
            let more =
                ...
                |> List.collect (fun path ->
                    ...
                    |> List.filter (fun next -> not (explored.Contains(next.EndState)) && List.length path.Path <= maxPathLength)
                )
            ...
            yield sortedPaths
            yield! next_search_step more explored endState maxPathLength
    }
...
let pathSets = next_search_step initialPaths initialExplored endState 7 //accept only answers with 8 steps
